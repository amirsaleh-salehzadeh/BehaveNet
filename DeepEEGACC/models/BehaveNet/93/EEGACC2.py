...
def build_model(X_train, row, cell):
    model = Sequential()
    model.add(Conv1D(8, 8, strides=4, data_format='channels_last',
             input_shape=(300, 5)))
    model.add(Conv1D(16, 4, strides=3, data_format='channels_last'))
    model.add(Conv1D(32, 1, strides=1, data_format='channels_last'))
    model.add(Dense(256))
