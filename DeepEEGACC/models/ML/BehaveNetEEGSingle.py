...
def build_model(X_train, row, cell):
    model = Sequential()
    model.add(Conv1D(32, 2, strides=1, data_format='channels_last',
             input_shape=(300, 1)))
    model.add(Activation("relu"))
    model.add(BatchNormalization())
    model.add(Conv1D(64, 4, strides=4, data_format='channels_last'))
    model.add(Activation("relu"))
    model.add(BatchNormalization())
    model.add(MaxPooling1D(pool_size=8, strides = 2, padding='valid'))
    model.add(Dense(512))
...
#     print(y_train.shape)
#     y_train = remove_column(y_train, 1)
#     print(y_train.shape)
#     y_train = remove_column(y_train, 1)
#     print(y_train.shape)
    epochs = 50  # 21
