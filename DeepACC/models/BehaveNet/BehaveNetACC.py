...
def build_model(X_train, row, cell):
    model = Sequential()
    model.add(Conv1D(32, 6, strides=1, data_format='channels_last',
             input_shape=(75, 3)))
    model.add(Activation("relu"))
    model.add(BatchNormalization())
    model.add(Conv1D(64, 9, strides=4, data_format='channels_last'))
    model.add(Activation("relu"))
    model.add(BatchNormalization())
    model.add(MaxPooling1D(pool_size=14, strides=2, padding='valid'))
    model.add(Dense(512))
