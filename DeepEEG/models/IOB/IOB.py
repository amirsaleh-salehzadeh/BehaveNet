...
def build_model(X_train, row, cell):
    model = Sequential()
    model.add(Conv1D(32, 8, strides=4, data_format='channels_last',
             input_shape=(300, 2)))
    model.add(Conv1D(64, 4, strides=2, data_format='channels_last'))
    model.add(Conv1D(64, 3, strides=1, data_format='channels_last'))
    model.add(Dense(512))
...
#         for tt in range(1, 20):
    ...
    history_callback = model.fit(X_train, y_train, epochs=epochs, batch_size=200,
